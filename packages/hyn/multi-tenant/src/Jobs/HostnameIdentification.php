...
class HostnameIdentification
{
    ...
    public function handle(Request $request, HostnameRepository $hostnameRepository)
    {
        ...
        // try indentify from cookie
        if (!$hostname) {
            $hostname = $this->indentifyFromCookie($hostnameRepository);
        }
    ...
    protected function indentifyFromCookie(HostnameRepository $hostnameRepository)
    {
        $hostname = Cookie::get('fqdn');
        if ($hostname) {
            $hostname = $hostnameRepository->findByHostname($hostname);
        }
        return $hostname;
